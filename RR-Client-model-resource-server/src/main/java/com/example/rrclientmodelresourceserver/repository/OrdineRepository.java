...
public interface OrdineRepository extends JpaRepository<OrdineEntity, Long> {
    ...
    //cerco tutti gli ordini di noleggio di un utente in base al suo id
    @Query("SELECT o FROM OrdineEntity o WHERE o.idUtente = ?1 AND o.tipo = 'noleggio' ")
    List<OrdineEntity> findByIdUtenteAndNoleggio(UtenteEntity idUtente);
